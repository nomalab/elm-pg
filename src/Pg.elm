...
jsonbFromDict: Dict String String -> Param
jsonbFromDict dict =
  jsonb (Json.Encode.object
    (List.map
      (\(k, v) -> (k, Json.Encode.string v)) -- (String, String) -> (String, Json.Encode.Value)
      (Dict.toList dict) -- List (String, String)
    ))
...
encodeParam: Param -> Json.Encode.Value
encodeParam param =
  case param of
    NullParam -> Encode.null
    IntParam i -> Encode.int i
    FloatParam f -> Encode.float f
    Double f -> Encode.float f
    StringParam s -> Encode.string s
    BoolParam b -> Encode.bool b
    Date d -> Date.encode d
    MaybeParam mParam -> Maybe.withDefault (Encode.null) (Maybe.map encodeParam mParam)
    ListParam list -> Encode.list (List.map encodeParam list)
    Jsonb p -> Encode.encode 0 p |> Encode.string
    Raw r -> r

encodeParams: List Param -> Encode.Value
encodeParams params =
  Encode.list <| List.map encodeParam params
...
createPool: PoolSettings -> Pool
createPool =
  Native.Pg.createPool

connect: Pool -> Task Error Client
connect =
  Native.Pg.connect

release: Client -> Task Never ()
release =
  Native.Pg.release
...
type Transaction = No | Yes

type alias Request a = Client -> Task Error a

type alias Response a = Task Error a
...
type alias QueryResultRaw = QueryResult Encode.Value

type alias QueryRaw = Query QueryResultRaw
...
-- FIXME we don't know when those fields might change...
